...
def make_dataset(split_file, split, root, num_classes=157):
    ...
    for vid in tqdm(data.keys()):
        ...
        if data[vid]['subset'] == split:
            if len(data[vid]['actions']) < 1:
                continue
            feature_path = os.path.join(root, vid + '.npy')
            if not os.path.exists(feature_path):
                missing_feature_count += 1
                continue
            fts = np.load(feature_path, mmap_mode='r')
    ...
    if missing_feature_count:
        print(
            'Skipped {} {} videos with missing feature files'.format(
                missing_feature_count, split
            )
        )
    return dataset
...
class Charades_2(data_utl.Dataset):

    def __init__(self, split_file, split, rgb_root, flow_root, batch_size, classes, num_clips, skip):

        self.data = make_dataset(split_file, split, rgb_root, classes)
        self.split = split
        self.split_file = split_file
        self.batch_size = batch_size
    ...
    def __getitem__(self, index):
        entry = self.data[index]
        rgb_feat = np.load(os.path.join(self.rgb_root, entry[0] + '.npy'))
        rgb_feat = rgb_feat.reshape((rgb_feat.shape[0], 1, 1, rgb_feat.shape[-1]))
        rgb_features = rgb_feat.astype(np.float32)
        # print(rgb_features.shape)


        flow_feat = np.load(os.path.join(self.flow_root, entry[0] + '.npy'))
        flow_feat = flow_feat.reshape((flow_feat.shape[0], 1, 1, flow_feat.shape[-1]))
        flow_features = flow_feat.astype(np.float32)

        if rgb_features.shape[0] < flow_features.shape[0]:
            flow_features = flow_features[0:rgb_features.shape[0], :, :, :]
        elif rgb_features.shape[0] > flow_features.shape[0]:
            rgb_features = rgb_features[0:flow_features.shape[0], :, :, :]

        features = np.array(torch.cat([torch.tensor(rgb_features), torch.tensor(flow_features)], dim=3))
        labels = entry[1]
        ...
        if self.split in ["training", "testing"]:
            if len(features) > num_clips and num_clips > 0:
                if self.split == "testing":
                    ...
                    random_index = random.choice(range(0, len(flow_features) - num_clips))
                features = features[random_index: random_index + num_clips: 1]
                labels = labels[random_index: random_index + num_clips: 1]
                hmap = hmap[random_index: random_index + num_clips: 1]
        # center_loc = np.transpose(center_loc, axes=[1, 0])
...
class collate_fn_unisize():
    ...
    def charades_collate_fn_unisize(self, batch):
        ...
        for b in batch:
            ...
            m[:b[0].shape[0]] = 1
            try:
                l[:b[0].shape[0], :] = b[1][:b[0].shape[0], :]
            except:
                print(b[0].shape[0])
                print(b[1].shape[0])
                print(b[2].shape[0])
            h[:b[0].shape[0], :] = b[2][:b[0].shape[0], :]

            # varied_length = True
            # if varied_length:
            #     f, m, l, h = f[0:length, :, :, :], m[0:length], l[0:length,:], h[0:length, :]
